...
void main() {
  ...
  test("Should add the 'Cross-Origin-Resource-Policy:same-origin' Header",
      () async {
    final handler =
        const Pipeline().addMiddleware(crossOriginResourcePolicy()).addHandler(
              (req) => syncHandler(
                req,
    ...
    expect(
      response.headers,
      containsPair('cross-origin-resource-policy', 'same-origin'),
    );
    expect(response.headers, containsPair('content-type', 'application/json'));
  });
  test("Should add the 'Cross-Origin-Resource-Policy:cross-origin' Header",
      () async {
    ...
    expect(
      response.headers,
      containsPair('cross-origin-resource-policy', 'cross-origin'),
    );
    expect(response.headers, containsPair('content-type', 'application/json'));
  });
  test("Should add the 'Cross-Origin-Resource-Policy:same-site' Header",
      () async {
    final handler = const Pipeline()
        .addMiddleware(
          crossOriginResourcePolicy(
    ...
    expect(
      response.headers,
      containsPair('cross-origin-resource-policy', 'same-site'),
    );
    expect(response.headers, containsPair('content-type', 'application/json'));
  });
}
